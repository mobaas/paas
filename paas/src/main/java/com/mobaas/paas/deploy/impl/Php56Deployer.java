...
public class Php56Deployer extends BaseDeployer {

	private static final Logger log = LoggerFactory.getLogger(Php56Deployer.class);
	
	@Override
	protected String getContainerImage() {
		return "registry-internal.cn-qingdao.aliyuncs.com/mobaas/php:5";
	}
	...
	protected void handlePaths(AppInfo appInfo, AppVersion ver, List<DeploymentVolume> volumeList) {
		...
		volumeList.add(new DeploymentVolume("home", String.format("%s/%s/%s/www", config.getAppsRoot(), appInfo.getAppId(), ver.getVersion()), "/var/www/html", null));
		if (!customUpload) {
			volumeList.add(new DeploymentVolume("upload", String.format("%s/%s/upload", config.getAppsRoot(), appInfo.getAppId()), "/var/www/html/upload/", "kfayun.upload-path"));
		}
	...
	public void handleFile(AppInfo appInfo, AppVersion ver) throws IOException {
		...
		Path target = Paths.get(config.getAppsRoot(), appInfo.getAppId(), ver.getVersion(), "www/html");
